function [ ranked_list_genes ] = prioritization( samples, F, F_opp, network_genes, choose_up_down_flag, rank_method_str)
    ...
    for j = 1:length(network_genes), % number of genes
		...
		rank_temp = rank/max(rank);
		final_weights_rho(j) = rhoScores(rank_temp);
        clear rank_temp;
    end
	
	%choose the aggregation method 
	if strcmp(rank_method_str, 'MEDIAN'),
        scores_used = final_weights_median;
	elseif strcmp(rank_method_str, 'SUM'),
        scores_used = final_weights_sum;		
    else
		scores_used = final_weights_rho;
    end
	
	%sort scores
	to_sort_rho = [scores_used ;1:length(scores_used)];
	to_sort_sorted_rho = sortrows(to_sort_rho');
	genes_pos_network_sorted_rho = to_sort_sorted_rho(:,2);	
	ranked_list_genes = network_genes(genes_pos_network_sorted_rho);
